...
//char one[5][13] = {"list", "end", "sort", "pop", "help", "sqrt" , "rec" , "neg" , "pow" , "+" , "-" , "*" , "/" };
//char two[5][4] = {"delete", "search", "peek", "push"};
//char three[6][2] = {"add", "insert"};

char *strlwr(char *buff)//เปลี่ยนสตริงเป็นพิมพ์เล็กโดยเปลี่ยนทีละตัว macOS มีปัญหาในการใช้ strlwr โดยตรง หากใช้ใน windows ไม่จำเป็นต้องมีส่วนตรงนี้
{
...
int check_parameter(char cmd[][20], int count, int *parameter)
{
    ...
    while ((index < count) && (sscanf(cmd[index], "%d%s", &check_int, &check_char) == 1))
        index++;
    *parameter = index - 1;
